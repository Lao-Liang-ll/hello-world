...
struct stud_node * InsertDoc (struct stud_node * head, struct stud_node* stud); /*插人*/
...
struct stud_node * DeleteDoc ( struct stud_node * head,int num)
{
	struct stud_node * ptr1,* ptr2;
		...
		if (head==NULL) /*链表空*/
			...
			/*要被删除结点为非表头结点*/
		ptr1=head;
		ptr2=head->next;/*从表头的下一个结点搜索所有符合删除要求的节点*/ 
		while (ptr2!=NULL) {
